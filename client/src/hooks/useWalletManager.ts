...
export function useWalletManager(): WalletState & {
  ...
  const connectWallet = useCallback(async (walletId: string) => {
    ...
    try {
      ...
      const selectedWallet = state.availableWallets.find(w => w.id === walletId);
      
      if (!isNetworkValid) {
        // Get detailed network information for error message
        const walletNetworkId = await wallet.getNetworkId();
        const expectedNetwork = await import('@/config/cardano').then(m => m.getCurrentNetworkConfig());
        const networkNames = await import('@/config/cardano').then(m => m.NETWORK_NAMES);
    ...
    const attemptAutoConnect = async () => {
      ...
      if (savedWalletId && !state.hasAttemptedAutoConnect) {
        ...
        try {
          ...
          setState(prev => {
            const selectedWallet = prev.availableWallets.find(w => w.id === savedWalletId);
            return {
              ...prev,
              isConnected: true,
              isConnecting: false,
          ...
          setState(prev => ({
            ...prev,
            isConnecting: false,
            error: error instanceof Error ? error.message : 'Auto-connection failed',
          }));
        }
      }
