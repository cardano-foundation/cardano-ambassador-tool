  ...
  const SocialLink: React.FC<{
    ...
    return (
      <a 
        ...
        className="flex items-center space-x-3 p-2 rounded-lg transition-all duration-200 hover:bg-gray-100 group cursor-pointer"
        aria-label={`Visit ${profileName}'s ${platform} profile`}
      >
        <div className="transition-transform duration-200 group-hover:scale-125">
          {icon}
        </div>
        <span className="text-neutral transition-colors duration-200 group-hover:text-primary-base">
          {text}
        </span>
      </a>
    );
