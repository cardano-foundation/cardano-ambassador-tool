'use client';
import { parseProposalDatum, extractWitnesses, extractRequiredSigners } from '@/utils';
import { storageApiClient } from '@/utils/storageApiClient';
import { ProposalData } from '@sidan-lab/cardano-ambassador-tool';
import type { AdminDecisionData, AdminDecision as AdminDecisionType } from '@types';
...
export default function Page({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    const shouldRefresh = isExplicitRefresh || (isMissing && !hasTriggeredSync.current);
  ...
  try {
    if (proposal?.plutusData) {
      ...
      proposalData = {
        ...
        url: metadata?.url || '',
        description: fetchedContent !== null ? fetchedContent : (metadata?.description || ''),
    ...
    if (proposal && proposalData.url) {
      const fetchContent = async () => {
        try {
          ...
          if (filename) {
            const res = await fetch(`/api/proposal-content?filename=${filename}`);
            if (res.ok) {
  ...
  if (dbLoading || (isSyncing && !proposal)) return <StateFeedback type="loading" />;
  ...
  return (
    <div className="container px-4 py-2 pb-8 sm:px-6">
      <div className="space-y-6">
        ...
        {adminDecisionData && (
          <AdminDecision adminDecisionData={adminDecisionData} proposal={proposal} />
        )}
      </div>
    </div>
  );
